...
use crate::default_message_id_fn;
...
fn unknown_topic_message_uses_default_message_id_fn() {
    ...
    let message_a = Message::new_with_seq_no_and_source(
        topic.clone(),
        b"test-payload".to_vec(),
        new_test_seqno(),
        remote_peer,
    );
    let message_b = Message::new_with_seq_no_and_source(
        topic.clone(),
        b"test-payload".to_vec(),
        new_test_seqno(),
        remote_peer,
    ...
    assert_matches!(&output_events[0], ServiceOut::MessageReceived { message_id, .. } => {
        let expected_message_id = default_message_id_fn(&message_a);
    ...
    assert_matches!(&output_events[1], ServiceOut::MessagePublished { message_id, .. } => {
        let expected_message_id = default_message_id_fn(&message_b);
        assert_eq!(message_id, &expected_message_id, "Message ID should have been generated using default message ID function");
    });
}
...
fn unsubscribed_topic_message_uses_default_message_id_fn() {
    ...
    let message_c = Message::new_with_seq_no_and_source(
        topic.clone(),
        b"test-payload".to_vec(),
        new_test_seqno(),
        remote_peer,
    ...
    assert_matches!(&output_events[2], ServiceOut::MessagePublished { message_id, .. } => {
        let expected_message_id = default_message_id_fn(&message_c);
        assert_eq!(message_id, &expected_message_id, "Message ID should have been generated using default message ID function");
    });
}
