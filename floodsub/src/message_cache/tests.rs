...
fn insert_and_check_if_contained() {
    ...
    //// When
    let result1 = cache.put(&id1, msg1.clone());
    let result2 = cache.put(&id2, msg2.clone());
    let result3 = cache.put(&id3, msg3.clone());

    // Insert a message with the same id as message 1
    let result4 = cache.put(&id1, msg1.clone());
...
fn iterate_over_cache_content() {
    ...
    let cache = {
        ...
        cache.put(&id1, msg1.clone());
        cache.put(&id2, msg2.clone());
        cache.put(&id3, msg3.clone());
...
fn insert_over_capacity() {
    ...
    let mut cache = MessageCache::with_capacity_and_ttl(capacity, ttl);
...
fn check_if_message_is_contained() {
    ...
    cache.put(&id4, msg4.clone());
    cache.put(&id5, msg5.clone());
...
fn insert_an_already_expired_message_should_update_the_timestamp() {
    ...
    let update_result = cache.put(&id2, msg2);
