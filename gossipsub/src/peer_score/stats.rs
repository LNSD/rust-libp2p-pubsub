...
use crate::error::MessageValidationError as ValidationError;
use crate::metrics::{Metrics, NoopMetrics, Penalty};
use crate::time_cache::TimeCache;
use crate::{MessageId, PeerScoreParams, TopicHash, TopicScoreParams};
...
impl PeerScore {
    ...
    pub(crate) fn score(&self, peer_id: &PeerId) -> f64 {
        let mut metrics: Box<dyn Metrics + Send> = Box::new(NoopMetrics::new());
        self.metric_score(peer_id, &mut metrics)
    }
