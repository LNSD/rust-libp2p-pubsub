...
impl<In: Message, Out> Encoder for Codec<In, Out> {
    ...
    fn encode(&mut self, item: Self::Item, dst: &mut BytesMut) -> Result<(), Self::Error> {
        let mut encoded_frame = BytesMut::with_capacity(item.encoded_len());
        item.encode(&mut encoded_frame)
            .expect("BytesMut to have sufficient capacity.");
        self.uvi
            .encode(encoded_frame.freeze(), dst)
...
impl<In, Out: Message + Default> Decoder for Codec<In, Out> {
    ...
    fn decode(&mut self, src: &mut BytesMut) -> Result<Option<Self::Item>, Self::Error> {
        ...
        Ok(bytes.map(|bytes| Message::decode(bytes)).transpose()?)
    }
}
